...
type Forecast = { column: string; periods: number; trend_per_period: number; forecast: number[]; note: string };
    ...
    <main style={{ maxWidth: 1000, margin: "0 auto", padding: "56px 24px" }}>
      <section style={cardStyle}>
        ...
        <form onSubmit={run} style={{ display: "grid", gap: 14, marginTop: 24 }}>
          ...
          <label style={labelStyle}>Numeric column<input value={column} onChange={(e) => setColumn(e.target.value)} placeholder="Revenue" style={inputStyle} /></label>
          <label style={labelStyle}>Forecast periods<input type="number" min={1} max={24} value={periods} onChange={(e) => setPeriods(Number(e.target.value))} style={inputStyle} /></label>
          <button type="submit" disabled={loading} style={buttonStyle}>{loading ? "Forecasting…" : "Generate Forecast"}</button>
        </form>
        {error && <p style={{ color: "#b42318" }}>{error}</p>}
      </section>
      {forecast && <section style={{ ...cardStyle, marginTop: 18 }}>
        <div style={tagStyle}>Linear trend</div>
        <h2 style={{ margin: "12px 0" }}>{forecast.column} forecast</h2>
        <p>Trend: <strong>{forecast.trend_per_period.toLocaleString()}</strong> per period</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(110px, 1fr))", gap: 10 }}>
          {forecast.forecast.map((value, index) => <div key={index} style={forecastCell}><small>Period {index + 1}</small><strong>{value.toLocaleString()}</strong></div>)}
        </div>
        <p style={{ color: "#667085", fontSize: 13, marginTop: 16 }}>{forecast.note}</p>
      </section>}
    </main>
  );
}
...
const forecastCell = { padding: 16, borderRadius: 14, background: "#f6f8fb", display: "grid", gap: 6 };
