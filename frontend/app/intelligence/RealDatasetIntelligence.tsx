...
import GroundedAnalystPanel from "../components/GroundedAnalystPanel";
...
export default function RealDatasetIntelligence() {
  ...
  useEffect(() => {
    ...
    async function runAnalysis() {
      try {
        ...
        const anomalyBody = anomalyResponse.ok ? await anomalyResponse.json() : null;
        const driverBody = driverResponse.ok ? await driverResponse.json() : null;
        const forecastBody = forecastResponse.ok ? await forecastResponse.json() : null;
        ...
        if (recommendationResponse.ok) setRecommendations((await recommendationResponse.json()).recommendations ?? []);
  ...
  return <div style={{display:"grid",gap:14}}>
    <section style={cardStyle}><div style={eyebrowStyle}>Active dataset</div><h2 style={{margin:"8px 0 4px"}}>{dataset.filename}</h2><p style={{margin:0,color:"#667085"}}>{dataset.profile.rows} rows · {dataset.profile.columns} columns · quality {dataset.profile.quality_score}/100</p><p style={{margin:"10px 0 0",fontSize:12,color:"#667085"}}>{status}</p></section>
    {analysis?.summary && <section style={cardStyle}><div style={eyebrowStyle}>Backend-calculated summary</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginTop:14}}>{[["Revenue",revenue],["Cost",analysis.summary.cost],["Profit",profit],["Quantity",analysis.summary.quantity]].filter(([,v])=>v !== undefined).map(([label,v])=><div key={String(label)} style={metricStyle}><span>{label}</span><strong>{Number(v).toLocaleString("en-IN")}</strong></div>)}</div></section>}
    {anomalies && <section style={cardStyle}><div style={eyebrowStyle}>Diagnostic screening</div><h2 style={{margin:"8px 0 4px"}}>{anomalies.anomaly_count} potential anomalies</h2><p style={{color:"#667085",marginTop:0}}>IQR screening across numeric preview columns. This is a review signal, not proof of an error or fraud.</p>{anomalies.findings?.slice(0,5).map((item:any,index:number)=><div key={index} style={findingStyle}><strong>{item.column}</strong><span>Row {item.row_index}</span><span>Value {Number(item.value).toLocaleString("en-IN")}</span></div>)}</section>}
    {drivers?.drivers?.length > 0 && <section style={cardStyle}><div style={eyebrowStyle}>Profit drivers</div><h2 style={{margin:"8px 0 4px"}}>What contributes most to profit?</h2><p style={{color:"#667085",marginTop:0}}>Descriptive contribution analysis across available dimensions.</p>{drivers.drivers.slice(0,6).map((item:any,index:number)=><div key={index} style={findingStyle}><strong>{item.label}</strong><span>{item.dimension}</span><span>₹{Number(item.value).toLocaleString("en-IN")}</span><span>{Number(item.share_pct).toFixed(1)}%</span></div>)}</section>}
    {forecast && <section style={cardStyle}><div style={eyebrowStyle}>Revenue outlook</div><h2 style={{margin:"8px 0 4px"}}>Baseline forecast: {forecast.trend}</h2><p style={{color:"#667085",marginTop:0}}>Three-step linear trend baseline. RMSE: ₹{Number(forecast.rmse).toLocaleString("en-IN", {maximumFractionDigits:0})}.</p><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{forecast.forecast.map((value:number,index:number)=><div key={index} style={metricStyle}><span>Period {index+1}</span><strong>₹{Number(value).toLocaleString("en-IN", {maximumFractionDigits:0})}</strong></div>)}</div><p style={{fontSize:12,color:"#667085",marginBottom:0}}>{forecast.note}</p></section>}
    <RecommendationPanel recommendations={recommendations}/>
    {analysis && <WhatIfSimulator revenue={revenue} profit={profit}/>}<IntelligenceSummary rows={dataset.preview}/><BusinessCharts rows={dataset.preview}/>
    {analysis && <GroundedAnalystPanel dataset={dataset} analysis={{...analysis, anomalies, forecast, recommendations: { recommendations }}}/>}
    {analysis && <AnalystPanel dataset={dataset} analysis={analysis}/>}<SuggestedQuestions/><InsightActions/>
  </div>;
}
