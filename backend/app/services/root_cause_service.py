...
def analyze_root_causes(df: pd.DataFrame, metric: str, top_n: int = 5) -> dict[str, Any]:
    ...
    for column in numeric.select_dtypes(exclude="number").columns:
        ...
        for category, value in grouped.items():
            ...
            dimensions.append({
                ...
                "contribution_pct": round((amount / total * 100) if total else 0.0, 2),
            })
    ...
    return {
        "metric": metric,
        "total": round(total, 2),
        "drivers": dimensions[: top_n * 3],
        "interpretation": "Drivers are ranked by absolute contribution to the selected metric; they indicate where to investigate, not causation by themselves.",
    }
